...
<script>
    ...
    $(document).ready(function() {
        ...
        div.innerHTML = "{{$toothList}}";
        var decodedJsonString = div.textContent;

        // Parse JSON string to JavaScript object
        var jsonArray = JSON.parse(decodedJsonString);
        // console.log(jsonArray);
        if (jsonArray) {
            localStorage.setItem('objectList', JSON.stringify(jsonArray));
        }
        $.each(jsonArray, function(index, item) {
            $('#' + item.tooth_number).addClass('blue-bg');
        });
    ...
    $('#form').submit(function(event) {
        ...
        var url = "{{ route('procedures.update', '#id') }}";
        var route = url.replace('#id', "{{ $item->id }}");
